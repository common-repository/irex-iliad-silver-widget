...
Using this widget you can easily place the IRex iLiad Silver containing an image of the cover of the book you are reading in the sidebar. You can also specify a URL to link to when clicking on the book cover image. Supports multiple instances, so you can use it multiple times in multiple sidebars. (adapted from the Simple Image Widget)

Once the plugin is enabled, the widget will be available in your widgets list as "IRex iLiad Silver Widget". You can add this widget to sidebars as many times as you need. The control interface allows you to specify the following options for each instance of the widget:
...
The IRex iLiad Silver Widget does not provide a mechanism for uploading images or files. You can however upload an image in the 'Write' section of WordPress. From there you can click on the 'Add an Image' icon (next to 'Add Media' label). After uploading an image, copy the 'Link URL' for use in your IRex iLiad Silver Widget.
...
`.irexiliadsilver { text-align: center; }
.irexiliadsilver { margin:0 auto; }
.irexiliadsilver { margin:0 auto; text-align: center; }
.irexiliadsilver { margin:0 auto; text-align: left; }
.irexiliadsilver { margin-top:0px; margin-right:0px; margin-bottom:0px; margin-left:0px; }`
